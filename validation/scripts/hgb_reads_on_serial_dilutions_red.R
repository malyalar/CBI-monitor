...
library(dplyr)
df <- subset(df, dilution__vv!=0)

df1.summary <- df %>%
  group_by(dilution_vv) %>%
  summarise(
    sd = sd(rp_value, na.rm = TRUE),
    mean = mean(rp_value)
  )
...
df2.summary <- subset(df2.summary, dilution_vv!=0)
...
p1 + geom_errorbar(width = 0.0) +
  geom_point(size = 2, shape=1,  color = "red") +
  scale_x_log10(breaks=c(0, 0.02,0.04,0.07,0.14,0.25,0.5,1)) +
  scale_y_log10() +
  geom_line(aes(group = 1),  color = "red") +
  labs(title = "Readouts on serial dilutions of blood", y= "[hemoglobin] (mmol/L)", x = expression(log[10]~(v/v~dilution))) +
  theme_pub() +
  theme(plot.title = element_text(vjust=2), panel.grid.major = element_blank(),  panel.grid.minor = element_blank())


ggsave("/Users/malyalar/Dropbox/Renal Transplant Research/hematuria monitor/figures/figures/raw_readouts_small_red.png",dpi=300, dev='png', height=5, width=5, units="in")
