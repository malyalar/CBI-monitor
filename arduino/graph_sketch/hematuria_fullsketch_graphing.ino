...
uint8_t graphColor = ST77XX_WHITE;
uint8_t pointColor = ST77XX_WHITE;
uint8_t lineColor = ST77XX_WHITE;
//String graphName = "Rate of change in load sensor";
byte graphRange = 120;
byte markSize = 3;

byte valueBlock[500];
uint8_t timeBlock[500];
byte locationBlock[500];
byte valuePos;
byte blockPos;
float temp;

//calculate values for graphing
#define numberOfMarks 8
#define originX 30
#define originY 200
#define sizeX 200
#define sizeY 150
...
byte boxSize = (sizeX / numberOfMarks);
...
const byte minorSizeY = (originY + 10);
const byte minorSizeX = (originX - 10);

byte numberSize = (sizeY / 6);
byte number[] = {numberSize, (numberSize * 2), (numberSize * 3), (numberSize * 4), (numberSize * 5), (numberSize * 6)};

byte numberValue = 20; //(graphRange / 6);
byte val[] = {graphRange, (numberValue * 5), (numberValue * 4), (numberValue * 3), (numberValue * 2), numberValue};
...
void drawGraph()
{
  ...
  tft.println(F("Rate of change in load sensor"));
  ...
  // draw lables
  for(byte i = 0; i < numberOfMarks; i++)
  {
...
void graph()
{
  ...
  timeBlock[valuePos] = ((millis()-3000) / 1000);
  ...
  */
  ...
  else
  {
    ...
    tft.fillRect((originX + 2), (originY - sizeY), sizeX, sizeY, ST77XX_BLACK);
    ...
    for(byte i = 0; i <= 7; i++)
    {
      ...
      tft.setTextColor(graphColor, ST77XX_BLACK);
      tft.setTextSize(1);
